...
// Default slides for immediate render
...
const HeroSection: React.FC<HeroSectionProps> = ({ data, isLoading: _isLoading = false }) => {
  ...
  const specialDealBanner = data?.specialDealBanner || null;
  const mattressBanner = data?.mattressBanner || null;
            ...
            <div className={`${styles.popupContent} ${styles.callbackContent}`} style={{ marginTop: '80px' }} onClick={(e) => e.stopPropagation()}>
              ...
              <form
                ...
                onSubmit={async (e) => {
                  ...
                  try {
                    const response = await API.post('/auth/contact/submit/', formDataObj);
