...
const AdminSixpineReturnRequests: React.FC = () => {
  ...
  const fetchReturnRequests = async () => {
    try {
      setLoading(true);
      const response = await orderAPI.getAdminSixpineReturnRequests();
      setReturnRequests(response.data || []);
    } catch (error) {
      console.error('Error fetching Sixpine return requests:', error);
      setReturnRequests([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleApproveReject = (request: ReturnRequest, approve: boolean) => {
    ...
    setAdminNotes(request.seller_notes || '');
    setShowModal(true);
  };
  ...
  const handleSubmitApproval = async () => {
    ...
    try {
      setProcessing(true);
      await orderAPI.approveAdminSixpineReturnRequest(selectedRequest.id, {
    ...
    <div className="admin-page">
      ...
      <div className="admin-page-header">
        <div className="admin-page-title">
          ...
          <div>
            <h1>Sixpine Return Requests</h1>
            <p className="admin-page-subtitle">Manage return requests for Sixpine products only</p>
          </div>
        </div>
      </div>
      ...
      {filteredRequests.length === 0 ? (
        <div className="admin-card">
          <div className="text-center py-5">
            <p className="text-muted">No return requests found for Sixpine products</p>
          </div>
        </div>
      ) : (
...
export default AdminSixpineReturnRequests;
